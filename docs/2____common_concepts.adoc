...
`Rx_Clock` variables must be `triggered input` Clocks and are meant to be connected by the importer to the corresponding `Tx_Clocks`.
For example, a `periodic` Clock indicates that the FMU can only send signals or frames periodically on a given time raster, while a `changing` or `countdown` Clock indicates that the FMU can send signals at varying time steps.
